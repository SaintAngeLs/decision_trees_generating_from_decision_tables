...
void func1() {
  ...
  /* is nulled */
  if (memcmp(&parsed_tree, (unsigned short[sizeof(StringTreeNode)]){0},
             sizeof(StringTreeNode)) == 0) {
    /*sfFont_destroy(font);*/
    return;
  }
  ...
  /*age 10*/
  DataQueryKey i0_0[] = {(DataQueryKey){0, DQInt},
  ...
  /*NR children*/
  DataQueryKey i0_2[] = {(DataQueryKey){0, DQInt},
                         (DataQueryKey){DQK_propId, DQKeyword},
                         (DataQueryKey){0, DQInt},
                         (DataQueryKey){DQK_propId, DQKeyword},
                         (DataQueryKey){1, DQInt},
  ...
  /*salary*/
  DataQueryKey i0_3[] = {(DataQueryKey){0, DQInt},
                         (DataQueryKey){DQK_propId, DQKeyword},
                         (DataQueryKey){0, DQInt},
                         (DataQueryKey){DQK_propId, DQKeyword},
                         (DataQueryKey){1, DQInt},
  ...
  /*marital status*/
  DataQueryKey i1_0[] = {(DataQueryKey){0, DQInt},
                         (DataQueryKey){DQK_propId, DQKeyword},
                         (DataQueryKey){0, DQInt},
                         (DataQueryKey){DQK_propId, DQKeyword},
                         (DataQueryKey){1, DQInt},
  ...
  char *titles_3[] = {
      "Age",       "States", "Children", "Marital status", "Multi_location",
      "Zip codes", "Salary"};
  ...
  int attr1[] = {0, 0, 1};
  int attr2[] = {0, 1};
  int attr3[] = {0, 0, 0, 0, 0, 1, 1};

  int *numAttr[4];
  int numAttr0[] = {1, 0, 1, 1};
  int numAttr1[] = {0, 1, 1};
  int numAttr2[] = {0, 0};
  int numAttr3[] = {1, 0, 1, 0, 0, 0, 1};
  ...
  int depth[4] = {999, 999, 999, 999};

  DecisionTable decision_tables[4];

  decision_tables[0] = create_decision_table_from_parsed_tree(
      parsed_tree, titles[0], instr_cases[0], instr_sizes_cases[0],
      nr_columns[0]);

  if (decision_tables[0].data == NULL) {
    free_string_tree_children(parsed_tree);
    return;
  }

  decision_tables[1] = create_decision_table_from_parsed_tree(
      parsed_tree, titles[1], instr_cases[1], instr_sizes_cases[1],
      nr_columns[1]);

  if (decision_tables[1].data == NULL) {
    free_string_tree_children(parsed_tree);
    free_decision_table(decision_tables[0], 0);
    return;
  }

  decision_tables[2] = create_decision_table_from_parsed_tree(
      parsed_tree, titles[2], instr_cases[2], instr_sizes_cases[2],
      nr_columns[2]);

  if (decision_tables[2].data == NULL) {
    free_string_tree_children(parsed_tree);
    free_decision_table(decision_tables[0], 0);
    free_decision_table(decision_tables[1], 0);
    return;
  }

  decision_tables[3] = create_decision_table_from_parsed_tree(
      parsed_tree, titles[3], instr_cases[3], instr_sizes_cases[3],
      nr_columns[3]);

  if (decision_tables[3].data == NULL) {
    free_string_tree_children(parsed_tree);
    free_decision_table(decision_tables[0], 0);
    free_decision_table(decision_tables[1], 0);
    free_decision_table(decision_tables[2], 0);
    return;
  }
  ...
  convert_column_to_real(decision_tables[0], 0);
  convert_column_to_real(decision_tables[0], 2);
  convert_column_to_real(decision_tables[0], 3);

  convert_column_to_real(decision_tables[1], 1);
  convert_column_to_real(decision_tables[1], 2);

  convert_column_to_real(decision_tables[3], 0);
  convert_column_to_real(decision_tables[3], 2);
  convert_column_to_real(decision_tables[3], 6);

  for (size_t i = 0; i < decision_tables[3].nr_rows; ++i) {
    if (decision_tables[3]
            .data[i * decision_tables[3].nr_columns + 4]
            .key.data.str.ptr[0] == '1') {
      decision_tables[3]
          .data[i * decision_tables[3].nr_columns + 4]
          .key.data.str.ptr[0] = '0';
    } else {
      decision_tables[3]
          .data[i * decision_tables[3].nr_columns + 4]
          .key.data.str.ptr[0] = '1';
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    TextTreeNode decision_tree =
        build_decision_tree(decision_tables[i], attr[i], numAttr[i],
                            i == 3 ? 2 : 1, depth[i], tgt_attr[i], NULL, NULL);
    ...
    if (!decision_tree.node_text) {
      ...
      free_decision_table(decision_tables[1], 0);
      free_decision_table(decision_tables[2], 0);
      free_decision_table(decision_tables[3], 0);
      return;
    }
    ...
    if (!memcmp(&viz, &(TextTreeVisualization){0},
                sizeof(TextTreeVisualization))) {
...
void func2() {
  ...
  DecisionTable decision_table = create_decision_table_from_parsed_tree(
      parsed_tree, titles_1, instr_attr1, instr_size1, 3);
  ...
  TextTreeNode decision_tree =
      build_decision_tree(decision_table, attr1, numAttr1, 1, 2, 2, NULL, NULL);
