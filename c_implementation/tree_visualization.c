...
void tree_visualization_draw(const TextTreeVisualization *tree,
  ...
  sfRenderWindow_drawPrimitives(window, tree->pointers,
                                tree->nr_pointers_lines * 2, sfLines, NULL);
  /*draw nodes*/
...
static TextTreeNode *first_dfs(TextTreeNode *root) { return root; }
...
TextTreeVisualization tree_visualization_create(TextTreeNode *tree,
  ...
  sfCircleShape **cspp =
      xcalloc(n, sizeof(sfCircleShape *), __LINE__, __FILE__);
  if (!cspp) {
    memset(&result, 0, sizeof(result));
    return result;
  }
  ...
  i = 0;
  for (TextTreeNode *node = next_dfs(first_dfs(tree)); node;
       ++i, node = next_dfs(node)) {
    sfText_setString(ptr_txtpp[i], node->parent_text);
    sfText_setPosition(ptr_txtpp[i],
                       (sfVector2f){node->ptr_text_x, node->ptr_text_y});
